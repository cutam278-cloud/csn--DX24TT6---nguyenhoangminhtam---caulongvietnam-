...
**Sinh vien:** `Nguyễn Hòang Minh Tâm - DX24TT6`
...
- Mời GVHD Dương Ngọc Vân Khanh trở thành collaborator

### 1.2. Phân tích yêu cầu
- Đọc kỹ chi tiết đề tài: website giới thiệu trung tâm cầu lông
- Xác định cấp độ dùng: HTML + CSS + JS static, không database
- List ra các trang cần làm: index (trang chủ), about (giới thiệu), contact (liên hệ), news (tin tức)

### 1.3. Ve ERD va Cấu trúc Database
- Tạo file `setup/erd.md` với Mermaid diagram
- Nếu sau này cần mở rộng backend + database, co schema san san
...
1. **Chọn Express.js thay vì static HTTP server** - chỉ dùng file, mà express cho phép thêm routing sau, có gọi để test responsive.

2. **Dùng Mermaid ERD và dùng database** - giai đọan này không cần backend/DB, mà dùng Mermaid để giữ mở khóa phát triển tiếp theo. Khi đọc ổn và GVHD phê duyệt, có thể thêm MySQL/PostgreSQL nhận trong tuần sau.
...
- Thử và demo với đểkhông bị lỗi
...
**Ghi chú:** Tuần tới sẽ đạt task HTML và CSS và ảnh đưa báo cáo qua email GVHD.
